...
from homeassistant.const import ATTR_ENTITY_ID, CONF_SERVICE, CONF_TYPE, STATE_PLAYING
...
from .const import (  # Config keys; Service keys (Inputs); Inner Channel keys; Entity IDs (auto-created); Other
    ...
    CONF_IS_VOICE,
    CONF_MESSAGE,
    CONF_OVERRIDE_GREETINGS,
    CONF_PERSON_ENTITIES,
    CONF_PRIORITY,
    CONF_PRIORITY_VOLUME,
    CONF_SERVICE,
    CONF_SKIP_ASSISTANT_NAME,
    CONF_SKIP_GREETING,
    CONF_TARGET,
    CONF_TARGET_DATA,
    CONF_TARGETS,
...
async def _apply_resume(hass: HomeAssistant, entity_id: str, target_volume: float):
    ...
    restore_volume = (
        snap["volume"] if snap and snap["volume"] is not None else target_volume
    )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    weekend_days = [
        ...
        for d in conf.get("weekend_days", ["5", "6"])
    ]
    ...
    async def voice_queue_worker():
        ...
        while True:
            ...
            try:
                ...
                _LOGGER.debug(
                    f"UniNotifier: Elaborazione messaggio vocale: {task['text_content'][:35]}..."
                )
                _LOGGER.debug(
                    f"UniNotifier: {task['physical_players']} {task['target_volume']} {task['service']} {task['payload']} "
                )
                await hass.services.async_call(
                    task["domain"], task["service"], task["payload"]
                )
                # 4. ATTESA FINE MESSAGGIO
                tts_buffer = hass.data[DOMAIN][entry.entry_id].get("tts_buffer", 2.5)
                wait_time = estimate_tts_duration(
                    task["text_content"], buffer=tts_buffer
                )
                _LOGGER.debug(
                    f"UniNotifier: Attesa di {wait_time:.2f}s per fine messaggio."
                )
                await asyncio.sleep(wait_time)
                # 5. CONTROLLO CODA PER RESUME
                if voice_queue.empty():
                    _LOGGER.debug(
                        "UniNotifier: Coda vuota, chiusura sessione e ripristino media."
                    )
                    for eid in list(_ORIGINAL_STATES.keys()):
                        await _apply_resume(hass, eid, target_vol)
                else:
                    _LOGGER.debug(
                        f"UniNotifier: Coda non vuota ({voice_queue.qsize()} messaggi), posticipo resume."
                    )
            except Exception as e:
                _ORIGINAL_STATES.clear()
    ...
    async def async_send_notification(call: ServiceCall):
        ...
        ignore_title_voice = call.data.get(
            CONF_IGNORE_TITLE_VOICE, global_ignore_title_voice
        )
        # 2. Analisi Contesto
        now = dt_util.now()
        now_time = now.time()
        ...
        if now_weekday in weekend_days:
            active_dnd = dnd_conf.get(
                "weekend", dnd_conf.get("weekday", {"start": "23:00", "end": "06:00"})
            )
        else:
            active_dnd = dnd_conf.get("weekday", {"start": "23:00", "end": "06:00"})
        is_dnd_active = is_time_in_range(
            active_dnd["start"], active_dnd["end"], now_time
        )
        ...
        dnd_override_entry = ent_reg.async_get_entity_id(
            "switch", DOMAIN, dnd_override_uid
        )
        dnd_override_state = (
            hass.states.get(dnd_override_entry) if dnd_override_entry else None
        )
        ...
        current_greeting = (
            random.choice(options) if options and not skip_greeting else ""
        )
        ...
        for target_alias in targets:
            ...
            dynamic_entities = specific_data.pop(
                CONF_ENTITY_ID, channel_conf.get(CONF_TARGET)
            )
            if isinstance(dynamic_entities, str):
                dynamic_entities = [
                    e.strip() for e in dynamic_entities.split(",") if e.strip()
                ]
            elif dynamic_entities is None:
                dynamic_entities = []
            _LOGGER.debug(
                f"UniNotifier: Sezione A, Dynamic Entities {dynamic_entities}"
            )

            ####################################################################
            # B. Selezione Servizio
            service_type = specific_data.pop(
                CONF_TYPE, runtime_data.get(CONF_TYPE, None)
            )
            ...
            _LOGGER.debug(
                f"UniNotifier: Sezione B, Full Service Name {full_service_name}"
            )
            ...
            if target_raw_message in COMPANION_COMMANDS or str(
                target_raw_message
            ).startswith("command_"):
                is_command_message = True
            _LOGGER.debug(
                f"UniNotifier: Sezione C, Is Command Message {is_command_message}"
            )
            ...
            if not parse_mode and not is_voice_channel:
                parse_mode = hass.data[DOMAIN][entry.entry_id].get(
                    "text_format", "html"
                )
            ...
            if is_command_message:
                ...
                if is_voice_channel:
                    ...
                    clean_msg = sanitize_text_visual(
                        str(target_raw_message), parse_mode
                    )
                    clean_greet = sanitize_text_visual(current_greeting, parse_mode)
                    clean_orig_title = (
                        sanitize_text_visual(final_title, parse_mode)
                        if final_title
                        else None
                    )
                    if use_bold_prefix:
                        ...
                        clean_orig_title = apply_formatting(
                            clean_orig_title, parse_mode, "bold"
                        )
                    prefix_parts = []
                    ...
                    clean_prefix = (
                        f"[{' - '.join(prefix_parts)}]" if prefix_parts else ""
                    )
                    greeting_part = f"{clean_greet}. " if clean_greet else ""
                    if clean_orig_title:
                        final_title = (
                            f"{clean_prefix} {clean_orig_title}"
                            if clean_prefix
                            else clean_orig_title
                        )
                        final_msg = f"{greeting_part}{clean_msg}"
                    else:
                        final_msg = (
                            f"{clean_prefix} {greeting_part}{clean_msg}"
                            if clean_prefix
                            else f"{greeting_part}{clean_msg}"
                        )
            ...
            if override_volume is not None:
                try:
                    target_volume = float(override_volume)
                except ValueError:
            ...
            if is_voice_channel:
                if is_dnd_active and not is_priority and override_volume is None:
                    _LOGGER.debug(
                        f"UniNotifier: Sezione F, Skipped '{target_alias}' (DND attivo)"
                    )
                    continue
            ...
            notification_mode = hass.data[DOMAIN][entry.entry_id].get(
            ...
            if person_entities_conf and notification_mode != "Normal":
                if notification_mode == "Voice home":
                    ...
                    family_home = (
                        family_state is not None and family_state.state == "home"
                    )
                    if family_home and not is_voice_channel:
                        _LOGGER.debug(
                            f"UniNotifier: Sezione F2, Skipped '{target_alias}' (Voice home, family a casa → solo voce)"
                        )
                        continue
                    elif not family_home and is_voice_channel:
                        _LOGGER.debug(
                            f"UniNotifier: Sezione F2, Skipped '{target_alias}' (Voice home, family non a casa → solo testo)"
                        )
                        continue
                elif notification_mode == "Text home":
                    if is_voice_channel:
                        _LOGGER.debug(
                            f"UniNotifier: Sezione F2, Skipped '{target_alias}' (Text home mode)"
                        )
                        continue
            ...
            _LOGGER.debug(
                f"UniNotifier: Sezione G, service_payload '{service_payload}' "
            )
            ...
            if conf_target_value is not None:
                ...
                if not isinstance(conf_target_value, list):
                    if isinstance(conf_target_value, str) and "," in conf_target_value:
                        conf_target_value = [
                            s.strip() for s in conf_target_value.split(",") if s.strip()
                        ]
                    else:
            ...
            _LOGGER.debug(
                f"UniNotifier: Sezione H, service_payload '{service_payload}' "
            )
            ...
            _LOGGER.debug(
                f"UniNotifier: Sezione I, service_payload '{service_payload}' "
            )
            if (
            ...
            if srv_domain == "telegram_bot":
                if conf_target_value:
                    for chat_id in conf_target_value:
                        ...
                        _LOGGER.debug(
                            f"UniNotifier: Sezione J, Telegram to chat_id={chat_id} payload {p}"
                        )
                ...
                if srv_domain == "tts":
                    ...
                    tts_media_players = specific_data.pop(
                ...
                _LOGGER.debug(
                    f"UniNotifier: Sezione J, Media players coinvolti {physical_players}."
                )
                queue_item = {
                    "domain": srv_domain,
                    "service": srv_name,
        ...
        try:
            if not last_msg_eid:
                raise ValueError(
                    f"Entity with unique_id {last_msg_uid} not found in registry"
                )
            await hass.services.async_call(
                "text",
                "set_value",
